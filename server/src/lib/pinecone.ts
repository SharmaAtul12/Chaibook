...
const indexName = process.env.PINECONE_INDEX ?? "chaibooklm";
...
async function waitForIndexReady(name: string) {
    ...
    // Check the index at most 30 times.
...
export async function upsertSourceVectors(
    ...
    // Inside this Pinecone index, I want to work specifically with the namespace belonging to this workspace.
    const namespace = index.namespace(workspaceId);

    // 100-record batching keeps individual Pinecone requests manageable.
    const batchSize = 100;
    for (let i = 0; i < records.length; i += batchSize) {
        // Insert the record if it doesn't exist, or update/replace it if it already exists with the same ID.

...
export async function uploadPdfToCloudinary(buffer: Buffer, filename: string,): Promise<CloudinaryUploadResult> {
    ...
    /**
     * Frontend File → HTTP multipart/form-data → Backend.
        Multer extracts the uploaded file and gives you req.file.buffer.
        Buffer contains the actual PDF bytes.
        Backend creates a new FormData because it is making a new HTTP request to Cloudinary.
        file + upload_preset + folder are the fields Cloudinary needs to process and organize the upload.
    */
